...
# Track announcements so they don't repeat
announced = set()
    ...
    for boss in bosses.values():
        ...
        if "nextSpawn" in boss and boss["nextSpawn"] != "Unknown":
            try:
                ...
                key_spawn = (boss["name"], next_spawn_time, "spawn")
                key_warn = (boss["name"], next_spawn_time, "warn")
                if 0 <= diff <= 60 and key_spawn not in announced:
                    spawns.append(boss["name"])
                    announced.add(key_spawn)
                elif 600 <= diff <= 660 and key_warn not in announced:
                    warnings.append(boss["name"])
                    announced.add(key_warn)
                if diff < -900:
                    expired.add(key_spawn)
                    expired.add(key_warn)
            except Exception:
                continue
        # Fixed schedule
        elif "schedule" in boss:
            for s in boss["schedule"]:
                ...
                key_spawn = (boss["name"], schedule_time, "spawn")
                key_warn = (boss["name"], schedule_time, "warn")
                if 0 <= diff <= 60 and key_spawn not in announced:
                    spawns.append(boss["name"])
                    announced.add(key_spawn)
                elif 600 <= diff <= 660 and key_warn not in announced:
                ...
                if diff < -900:
                    expired.add(key_spawn)
                    expired.add(key_warn)
    announced.difference_update(expired)
    if warnings:
